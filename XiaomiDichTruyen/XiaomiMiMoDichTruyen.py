...
# Giá Overseas (USD / 1M tokens) theo bảng Pricing & Rate Limits của MiMo.
# Ứng dụng không dùng cache nên dùng mức Input (Cache Miss).
MODEL_PRICING = {
    "mimo-v2.5-pro": {"input_per_1m": 1.00, "output_per_1m": 3.00},
    "mimo-v2-pro": {"input_per_1m": 1.00, "output_per_1m": 3.00},
    "mimo-v2.5": {"input_per_1m": 0.40, "output_per_1m": 2.00},
    "mimo-v2-omni": {"input_per_1m": 0.40, "output_per_1m": 2.00},
    "mimo-v2-flash": {"input_per_1m": 0.10, "output_per_1m": 0.30},
}
...
SETTINGS_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), "app_settings_mimo.json")
HISTORY_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), "translation_history_mimo.json")
...
# ================= GỌI API MIMO =================
def mimo_chat_completion(api_key, model_id, messages, temperature, max_output_tokens):
    payload = {
        "model": model_id,
        "messages": messages,
    ...
    try:
        ...
        try:
            ...
            pass
        raise RuntimeError(f"HTTP {e.code}: {error_body or e.reason}")
...
def start_translation():
    ...
    api_key = api_key_entry.get().strip()
    if not api_key:
        messagebox.showerror("Lỗi", "Vui lòng nhập Xiaomi MiMo API Key.")
        return
    ...
    try:
        ...
        if max_output_tokens < 256 or max_output_tokens > 131072:
            raise ValueError
    except Exception:
        messagebox.showerror("Lỗi", "Max output tokens phải là số nguyên từ 256 đến 131072.")
        return
...
# ================= GUI (Giao diện) =================
root = tk.Tk()
root.title("📖 App Dịch Truyện – Powered by Xiaomi MiMo")
...
ttk.Label(header_title_frame, text="App Dịch Truyện – Xiaomi MiMo", style="Header.TLabel").pack(anchor="w")
ttk.Label(header_title_frame, text="Giao diện quen thuộc, chỉ đổi provider sang MiMo.", style="SubHeader.TLabel").pack(anchor="w", pady=(4, 6))
...
for text, color in [("Xiaomi MiMo API", PALETTE["accent"]), ("OpenAI-Compatible Chat", PALETTE["accent_alt"])]:
...
card_api = build_card(translate_tab, "🔐 Xiaomi MiMo API Key", 0, 1, colspan=2)
tk.Label(card_api, text="Dùng API key từ MiMo Console. Key được mã hóa và chỉ dùng trên máy này.", bg=PALETTE["panel"], fg=PALETTE["text_muted"], font=("Segoe UI", 9)).grid(row=1, column=0, sticky="w", pady=(0, 6))
...
tk.Label(card_api, text="Console: platform.xiaomimimo.com › API Keys", bg=PALETTE["panel"], fg=PALETTE["accent_alt"], font=("Segoe UI", 9, "bold")).grid(row=3, column=0, sticky="w", pady=(4, 0))
...
tk.Label(card_config, text="Nhiệt độ (0-1.5)", bg=PALETTE["panel"], fg=PALETTE["text_muted"], font=("Segoe UI", 9, "bold")).grid(row=4, column=0, sticky="w")
temp_scale = ttk.Scale(card_config, from_=0.0, to=1.5, variable=temp_var, orient="horizontal", length=200, style="Accent.Horizontal.TScale")
